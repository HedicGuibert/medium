                        ...
                        <div class="d-flex row align-content-between h-100 ">
                            <div>
                                <div class="d-flex justify-content-between">
                                    <p>20/06/2018</p>
                                    @auth
                                        <a dusk="add_to_favorite_{{ $article->slug }}" style="cursor: pointer" onclick="addRemoveFavourite(@if($article->isFavourite() == false) false @else true @endif, {{ $article->id }})"><i id="{{ $article->id }}"  class="@if($article->isFavourite() == false) icon-heart2 @else icon-heart @endif fs-20 mb-1 text-purple"></i></a>
                                    @endauth
                                </div>
                                <h3>{{ $article->title }}</h3>
